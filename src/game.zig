...
const c = @import("celestial_system.zig");
...
pub fn start(ecs: *ECS) !void {
    ...
    _ = try ecs.registerSystem(c.CelestialSystem);
...
fn createSun(ecs: *ECS, position: raylib.Vector2) !void {
    var eID = try ecs.getSystem(c.CelestialSystem).?.spawnCelestial(
    ...
    var celestial = ecs.getOnePtr(eID, c.Celestial);
    celestial.?.body.enabled = false;
}

fn createPlanet(ecs: *ECS, position: raylib.Vector2, velocity: raylib.Vector2) !void {
    _ = try ecs.getSystem(c.CelestialSystem).?.spawnCelestial(
